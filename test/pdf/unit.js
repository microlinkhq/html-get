...
const { identifierDate, documentYear } = require('../../src/pdf/date')
const { getDescription } = require('../../src/pdf/description')
const { getTitle, isByline } = require('../../src/pdf/title')
const { isBannerLine, isPersonName, splitNamePairs } = require('../../src/pdf/text')
const { publisherFromLine, publisherFromUrl, isVenue } = require('../../src/pdf/publisher')
const { readEmbedded, toDate } = require('../../src/pdf/embedded')
const { nameCount, toAuthor } = require('../../src/pdf/author')
const { getLang } = require('../../src/pdf/lang')
const { getMedia } = require('../../src/pdf/media')
...
test('a byline survives emails, superscripts and editors', t => {
  t.is(toAuthor([line(0, 10, 'Nitish Srivastava nitish@cs.toronto.edu')], [0]), 'Nitish Srivastava')
  t.is(
    toAuthor([line(0, 10, 'Loredana Bellantuono1,2, Flaviana Palmisano3')], [0]),
...
test('a venue is read out of the running header', t => {
  t.is(
    publisherFromLine('Scientific Reports | (2023) 13:1234 | https://doi.org/10.1038/x'),
    'Scientific Reports'
  )
  t.is(publisherFromLine('4 3 6 | N A T U R E | V O L 5 2 1 | 2 8 M A Y 2 0 1 5'), 'NATURE')
  ...
  t.true(isVenue('Frontiers in Psychology', 'https://www.frontiersin.org/x/pdf'))
  t.false(isVenue('official NBER publications', 'https://www.nber.org/x.pdf'))
})

test('the identifier dates the document', t => {
  t.is(identifierDate('https://arxiv.org/pdf/1706.03762v7'), '2017-06-01T00:00:00.000Z')
  t.is(identifierDate('https://arxiv.org/pdf/cs/0102004v1'), '2001-02-01T00:00:00.000Z')
  t.is(
  ...
  t.is(
    documentYear([line(0, 10, 'Published as a conference paper at ICLR 2021')], {
...
test('lang comes from the host or the words on the page', t => {
  t.is(
    getLang('We study the staggered introduction of a generative AI assistant', {
      url: 'https://example.com/paper.pdf'
    }),
    'en'
  )
  t.is(
    getLang('Enseñanza del inglés en secundaria para los estudiantes de la escuela', {
...
test('a small square image is the logo; otherwise the host favicon', t => {
  ...
  const { image, logo } = getMedia([{ width: 16, height: 16, channels: 3, data: pixels }], {
