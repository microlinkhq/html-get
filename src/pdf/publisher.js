...
const matchesHost = (value, url) => {
  ...
  return (
    comparable(value).startsWith(comparable(host)) || comparable(host).startsWith(comparable(value))
  )
}

const isVenue = (value, url) =>
  Boolean(value) && (VENUE_OPENING.test(value) || VENUE_MARK.test(value) || matchesHost(value, url))
...
const publisherFromLine = text =>
  flatten(collapseLetterSpacing(text).replace(RIGHTS_NOTICE, ' '))
    ...
    .filter(part => {
      ...
      return /\p{L}{3}/u.test(part) && !words.every(word => STRUCTURAL_WORD.test(word))
    })[0] || null

const sameText = (left, right) => Boolean(left && right) && comparable(left) === comparable(right)
...
const getPublisher = (lines, { url, title, author }) => {
  const venue = lines
    ...
    .find(value => isVenue(value, url) && isUsablePublisher(value, { author, title }))
