...
const stripNoise = text =>
  tidy(text.replace(EMAIL_GLOBAL, ' ').replace(FOOTNOTE_MARKS, ' ').replace(SUPERSCRIPTS, ''))

const isShouting = text => text === text.toUpperCase() && /\p{Lu}/u.test(text)

const isCapitalized = word => /^[\p{Lu}]/u.test(word) || NAME_PARTICLES.has(word.toLowerCase())

const isInvertedName = text => {
  if (!INVERTED_NAME.test(text) || text.split(/\s+/).length > MAX_INVERTED_WORDS) {
    return false
  }
...
const isPersonName = rawText => {
  ...
  if (isShouting(text) || COUNTRY_ABBREVIATION.test(text) || PLACE_NAME.test(text)) {
    return false
  }
  if (ORGANIZATION_WORDS.test(text) || FUNCTION_WORDS.test(text)) return false
  const words = text.split(/\s+/)
  return words.length >= 2 && words.length <= MAX_NAME_WORDS && words.every(isCapitalized)
}
...
const splitNamePairs = text => {
  ...
  if (words.length < MIN_RUN_WORDS || words.length > MAX_RUN_WORDS || words.length % 2 !== 0) {
    return []
  }
  if (!words.every(word => /^\p{Lu}/u.test(word))) return []
  return words.flatMap((word, index) => (index % 2 === 0 ? [`${word} ${words[index + 1]}`] : []))
}

const splitNameRun = part => {
  if (part.split(/\s+/).length < MIN_RUN_WORDS) return [part]
  const pieces = part.split(NAME_RUN_BOUNDARY)
  return pieces.every(piece => /^\p{Lu}/u.test(piece) && piece.split(/\s+/).length <= 3)
    ? pieces
    : [part]
}

const splitNames = text =>
  isInvertedName(text) ? [text] : text.split(/\s{2,}|\s*(?:,|;| and | & )\s*/).flatMap(splitNameRun)
