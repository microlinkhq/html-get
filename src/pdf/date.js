...
const ARXIV_NEW = /arxiv\.org\/(?:abs|pdf)\/(\d{2})(\d{2})\.\d{4,5}|arxiv:(\d{2})(\d{2})\.\d{4,5}/i
const ARXIV_OLD =
...
const MARKED_YEAR = [
  ...
  new RegExp(`${YEAR}\\s+(?:ieee|acm|international conference|conference on)`, 'i')
]
...
const identifierDate = (url, { text = '' } = {}) => {
  ...
  if (stamp && stamp.year >= ARXIV_EPOCH && stamp.month >= 1 && stamp.month <= 12) {
...
const documentYear = (lines, { embedded = {}, now = new Date() } = {}) => {
  ...
  return firstMarkedYear(headerAndFooter(lines), currentYear) || firstMarkedYear(venue, currentYear)
}
