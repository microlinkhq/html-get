...
const findTitleLine = lines => {
  for (const size of distinctSizes(lines).slice(0, TITLE_SIZE_LEVELS)) {
    const line = lines.find(
      candidate =>
        candidate.size === size && !isBannerLine(candidate.text) && !isByline(candidate, lines)
    )
    if (line) return line
  }
  return null
...
const isUsableTitle = text =>
  typeof text === 'string' && text.length >= MIN_TITLE_LENGTH && text.length <= MAX_TITLE_LENGTH

const getTitle = lines => {
  const line = findTitleLine(lines)
  if (!line) return null
  const block = titleLines(lines, line)
  const text = flatten(block.map(entry => entry.text.replace(LINE_NUMBER, '')).join(' '))
  return isUsableTitle(text) ? { text, indexes: block.map(entry => entry.index) } : null
}
